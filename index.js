...
let tradeStats = new Map(); // NEW: Track successful trades per user
let sellerRatings = new Map(); // NEW: Track seller ratings
let ticketClaims = new Map(); // NEW: Track who claimed which ticket
let ticketTimers = new Map(); // NEW: Track countdown timers

async function loadData() {
  if (!JSONBIN_API_KEY || !JSONBIN_BIN_ID) {
    console.log('⚠️ JSONBin not configured');
    return { ticketCategories: new Map(), orderChannels: new Map(), doneChannels: new Map(), adminUsers: new Map(), ticketChannels: new Map(), webCategories: new Map(), shopListings: new Map(), ticketOwners: new Map(), shopCategories: new Map(), transcriptChannels: new Map(), tradeChannels: new Map(), shopNews: new Map(), gameCategories: new Map(), tradeStats: new Map(), sellerRatings: new Map(), ticketClaims: new Map(), ticketTimers: new Map() };
  }
      ...
      res.on('end', () => {
        try {
          ...
          resolve({
            ...
            tradeStats: new Map(Object.entries(record.tradeStats || {})),
            sellerRatings: new Map(Object.entries(record.sellerRatings || {}).map(([k, v]) => [k, v || { total: 0, count: 0, ratings: [] }])),
            ticketClaims: new Map(Object.entries(record.ticketClaims || {})),
            ticketTimers: new Map(Object.entries(record.ticketTimers || {}))
          });
        } catch (err) {
          console.error('Error parsing data:', err.message);
          resolve({ ticketCategories: new Map(), orderChannels: new Map(), doneChannels: new Map(), adminUsers: new Map(), ticketChannels: new Map(), webCategories: new Map(), shopListings: new Map(), ticketOwners: new Map(), shopCategories: new Map(), transcriptChannels: new Map(), tradeChannels: new Map(), shopNews: new Map(), gameCategories: new Map(), tradeStats: new Map(), sellerRatings: new Map(), ticketClaims: new Map(), ticketTimers: new Map() });
        }
      });
    });
...
async function saveData() {
  ...
  const data = {
    ...
    tradeStats: Object.fromEntries(tradeStats),
    sellerRatings: Object.fromEntries(sellerRatings),
...
client.once('ready', async () => {
  ...
  tradeStats = loadedData.tradeStats;
  sellerRatings = loadedData.sellerRatings;
  ticketClaims = loadedData.ticketClaims;
  ticketTimers = loadedData.ticketTimers;
});
      ...
      if (interaction.customId === 'ticket_modal') {
        ...
        try {
          ...
          const welcomeEmbed = new EmbedBuilder()
            ...
            .setDescription(`Thank you for creating a ticket! Our team will be with you shortly.\n\n━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━`)
            .addFields(
              { name: '📋 Your Request', value: `\`\`\`${serviceDescription}\`\`\``, inline: false },
              { name: '⏰ Created', value: `<t:${Math.floor(Date.now() / 1000)}:R>`, inline: true },
              { name: '👤 Customer', value: `${interaction.user}`, inline: true },
              { name: '📌 Status', value: '🟡 **Waiting for Staff**', inline: false }
            )
            .setThumbnail(interaction.user.displayAvatarURL())
            .setFooter({ text: '💡 Please wait patiently. Staff will respond soon!' })
            .setTimestamp();
