    ...
    client.on('interactionCreate', async (interaction) => {
      if (interaction.isButton()) {
        ...
        if (interaction.customId === 'shop_manage') {
          ...
          interaction.reply({ content: '🛒 **Manage:**', components: [row], ephemeral: true });
        }
        ...
        if (interaction.customId === 'shop_remove') {
          ...
          const row = new ActionRowBuilder().addComponents(selectMenu);
          interaction.reply({ content: '🗑️ **Select:**', components: [row], ephemeral: true });
        }
        ...
        if (interaction.customId === 'create_ticket') {
          ...
          if (!categoryId) return interaction.reply({ content: '❌ Not set!', ephemeral: true });
          const category = interaction.guild.channels.cache.get(categoryId);
          if (!category) return interaction.reply({ content: '❌ Not found!', ephemeral: true });
          ...
          if (existingTicket) return interaction.reply({ content: `❌ Ticket exists: <#${existingTicket.id}>`, ephemeral: true });
    ...
    ⏰ <t:${currentTimestamp}:F>`;
      ...
      if (interaction.isModalSubmit()) {
          ...
          try {
            ...
            if (orderChannelId) {
              ...
              if (orderChannel) {
                ...
                const orderEmbed = new EmbedBuilder().setColor('#FF6B35').setTitle('📦 New Order!').setDescription(`**${interaction.user.tag}**\n\n${serviceDescription}`).addFields({ name: '⏰ Time', value: `<t:${orderTimestamp}:F>`, inline: false }).setTimestamp();
                await orderChannel.send({ embeds: [orderEmbed] });
              }
            }
            interaction.reply({ content: `✅ Created! <#${ticketChannel.id}>`, ephemeral: true });
          } catch (err) {
        ...
        if (interaction.customId.startsWith('shop_add_modal_')) {
          ...
          if (newsChannelId) {
            ...
            if (newsChannel) {
              const newsEmbed = new EmbedBuilder().setColor('#00FF00').setTitle('🆕 New Item!').setDescription(`**${itemName}**\n\n🎮 ${gameCategory}\n💰 ${itemPrice}\n📦 ${itemStock}\n👤 ${interaction.user}`).setTimestamp();
              try {
          ...
          interaction.reply({ content: `✅ Added **${itemName}**!`, ephemeral: true });
        }
        ...
        if (interaction.customId.startsWith('shop_change_modal_')) {
          ...
          shopListings.set(interaction.guild.id, guildShops);
          await saveData();
          interaction.reply({ content: `✅ Updated **${itemName}**!`, ephemeral: true });
        }
      }
      ...
      if (interaction.isStringSelectMenu()) {
          ...
          for (const [userId, items] of guildShops) {
            ...
            for (const item of items) {
              if (item.gameCategory === selectedGame && (item.stock || 0) > 0 && optionCount < 25) {
                selectOptions.push({ label: `${item.name} - ${item.price} (${item.stock || 0})`, description: `Seller: ${userName}`, value: `${userId}-${item.id}` });
                optionCount++;
              }
            }
          }
          if (selectOptions.length === 0) return interaction.update({ content: `❌ No items for **${selectedGame}**!`, components: [] });
          const selectMenu = new StringSelectMenuBuilder().setCustomId('shop_select_item').setPlaceholder('Select item').addOptions(selectOptions);
          const row = new ActionRowBuilder().addComponents(selectMenu);
          await interaction.update({ content: `🎮 **${selectedGame}:**\nSelect item:`, components: [row] });
        }

        if (interaction.customId === 'shop_add_select_game') {
          const selectedGame = interaction.values[0];
          const modal = new ModalBuilder().setCustomId(`shop_add_modal_${selectedGame}`).setTitle(`Add to ${selectedGame}`);
        ...
        if (interaction.customId === 'shop_select_item') {
          ...
          if (!categoryId) return interaction.reply({ content: '❌ Not set! Ask admin `!conshop`', ephemeral: true });
          try {
            ...
            const doneButton = new ButtonBuilder().setCustomId(`shop_buyer_mark_done_${sellerId}_${buyer.id}_${itemId}`).setLabel('Mark Done').setEmoji('✅').setStyle(ButtonStyle.Success);
            const closeButton = new ButtonBuilder().setCustomId('close_ticket').setLabel('Close').setEmoji('🔒').setStyle(ButtonStyle.Danger);
            const row = new ActionRowBuilder().addComponents(doneButton, closeButton);
            const itemEmbed = new EmbedBuilder().setColor('#FFD700').setTitle('🛍️ Shop Transaction').setDescription(`**Steps:**\n1️⃣ Buyer clicks "Mark Done" after receiving\n2️⃣ Buyer confirms received\n3️⃣ Admin verifies\n\n━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━`).addFields({ name: '👤 Buyer', value: `${buyer}`, inline: true }, { name: '💼 Seller', value: `<@${sellerId}>`, inline: true }, { name: '\u200b', value: '\u200b', inline: true }, { name: '🎮 Game', value: `${item.gameCategory || 'N/A'}`, inline: true }, { name: '🛍️ Item', value: `${item.name}`, inline: true }, { name: '💰 Price', value: `${item.price}`, inline: true }, { name: '📦 Stock', value: `${item.stock}`, inline: false }).setFooter({ text: '⚠️ Only buyer can mark done' }).setTimestamp();
            await ticketChannel.send({ content: `${buyer} <@${sellerId}>`, embeds: [itemEmbed], components: [row] });
            interaction.update({ content: `✅ Created! <#${ticketChannel.id}>`, components: [] });
          } catch (err) {
            interaction.reply({ content: '❌ Failed!', ephemeral: true });
          }
        }
